...
pub struct CostInput {
    pub on_behalf_of_user_id: Option<i32>,
    pub name: String,
    pub amount: Decimal,
            ...
            while debtor.net_tally < Decimal::from(0u8) {
                ...
                if creditor_done {
                    ...
                    left_slice = left_slice
                        .split_last_mut()
                        .expect("because we checked above that last exists")
                        .1;
                }
            }
        }
...
fn test_balance() {
    ...
    let res = BalancingTransaction::from_debt_table(vec![
        UserDebt {
            user_id: 1,
            to_pay: dec!(0.50),
            to_recieve: dec!(15.50),
        },
        UserDebt {
            user_id: 2,
            to_pay: dec!(0.50),
            to_recieve: dec!(35.50),
        },
        UserDebt {
            user_id: 3,
            to_pay: dec!(5.50),
            to_recieve: dec!(55.50),
        },
        UserDebt {
            user_id: 4,
            to_pay: dec!(75.50),
...
impl Into<Option<Upload>> for DBUpload {
    fn into(self) -> Option<Upload> {
        ...
        match (id, extension, original_filename, size_kb) {
            (Some(id), Some(extension), Some(original_filename), Some(size_kb)) => Some(Upload {
                id,
                extension,
                original_filename,
                size_kb,
...
impl From<Option<Upload>> for DBUpload {
    fn from(opt: Option<Upload>) -> Self {
        match opt {
            Some(upl) => DBUpload {
                id: Some(upl.id),
                extension: Some(upl.extension),
                original_filename: Some(upl.original_filename),
                size_kb: Some(upl.size_kb),
            },
            None => DBUpload {
                id: None,
                extension: None,
...
impl Into<Option<CostShare>> for DBCostShare {
    fn into(self) -> Option<CostShare> {
        ...
        match (cost_id, debtor_id, paid) {
            (Some(cost_id), Some(debtor_id), Some(paid)) => Some(CostShare {
                cost_id,
                debtor_id,
                paid,
            }),
...
impl From<Option<CostShare>> for DBCostShare {
    fn from(opt: Option<CostShare>) -> Self {
        match opt {
            Some(upl) => DBCostShare {
                cost_id: Some(upl.cost_id),
                debtor_id: Some(upl.debtor_id),
                paid: Some(upl.paid),
            },
            None => DBCostShare {
                cost_id: None,
