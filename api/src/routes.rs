...
use super::auth::{Identity, WGMemberIdentity};
use crate::file_uploads::{delete_unreferenced_upload, multipart_parse, TempUpload};
use crate::{change_upload, db, DB_POOL};
use common::*;
...
async fn get_user_me(mut identity: Identity) -> impl Responder {
    ...
    HttpResponse::Ok().json(identity.0)
}
...
async fn put_user_me(identity: Identity, payload: Multipart) -> Result<impl Responder, Error> {
    ...
    let mut lmaobozo =
        multipart_parse(payload, ["name", "bio", "username"], ["profile_pic"]).await?;
    trace!("Bozo fields: {:?}", lmaobozo);

    if let Some(profile_picf) = &mut lmaobozo.1[0] {
        let new_upl = change_upload!("users", "profile_pic", i32)(
            profile_picf.move_responsibility(),
            identity.id,
            None,
        )
        .await;
...
async fn get_wg_public(params: web::Path<(String,)>) -> Result<impl Responder, DatabaseError> {
    ...
    Ok(HttpResponse::Ok().json(wg))
}

#[put("/my_wg")]
async fn put_wg(
    WGMemberIdentity { wg_id, .. }: WGMemberIdentity,
    payload: Multipart,
    ...
    // Get Multipart Fields
    let mut lmaobozo = multipart_parse(
        payload,
    ...
    if let Some(profile_picf) = &mut lmaobozo.1[0] {
        let new_upl = change_upload!("wgs", "profile_pic", i32)(
            profile_picf.move_responsibility(),
            wg_id,
            None,
        )
    ...
    if let Some(header_picf) = &mut lmaobozo.1[1] {
        let new_upl = change_upload!("wgs", "header_pic", i32)(
            header_picf.move_responsibility(),
            wg_id,
            None,
        )
        .await;
    ...
    if let Some(description) = &lmaobozo.0[2] {
        let res = sqlx::query!(
            "UPDATE wgs SET description=$1 WHERE id=$2",
            description,
            wg_id
        )
        .execute(db!())
...
#[get("/my_wg/costs")]
async fn get_wg_costs(
    member: WGMemberIdentity,
    query: web::Query<BalanceInput>,
) -> Result<impl Responder, DatabaseError> {
    let cost =
        Cost::get_all_balance(member.identity.id, member.wg_id, query.balance.unwrap_or(0)).await?;

    Ok(HttpResponse::Ok().json(cost))
}

#[post("/my_wg/costs")]
async fn post_wg_costs(
    WGMemberIdentity { identity, wg_id }: WGMemberIdentity,
    new_cost: web::Json<CostInput>,
) -> Result<impl Responder, DatabaseError> {
    ...
    for (debtor_user_id, debtor_has_paid) in new_cost.debtors.iter() {
        if !users.contains(&debtor_user_id) {
            continue;
        }
        let mut paid = *debtor_has_paid;
        if creditor_id == *debtor_user_id {
            paid = true;
        }
    ...
    Ok(HttpResponse::Ok().json(cost_id))
}

#[get("/my_wg/costs/{id}/detail")]
async fn get_wg_costs_id(
    member: WGMemberIdentity,
    params: web::Path<(i32,)>,
) -> Result<impl Responder, DatabaseError> {
...
async fn get_wg_costs_id_shares(
    member: WGMemberIdentity,
    params: web::Path<(i32,)>,
) -> Result<impl Responder, DatabaseError> {
...
async fn put_wg_costs_id_receit(
    identity: Identity,
    payload: Multipart,
    params: web::Path<(i32,)>,
) -> Result<impl Responder, Error> {
    let id: i32 = sqlx::query_scalar!("SELECT creditor_id FROM costs WHERE id=$1", params.0)
        .fetch_one(db!())
        .await
        .handle()?;
    ...
    if let Some(receitf) = &mut lmaobozo.1[0] {
        let new_upl = change_upload!("costs", "receit_id", i32)(
            receitf.move_responsibility(),
            (*params).0,
            identity.wg,
        )
        .await
    ...
    Ok(
        HttpResponse::BadRequest()
            .body("Please provide a 'receit' field using multipart form data"),
    )
}

#[delete("/my_wg/costs/{id}")]
async fn delete_wg_costs_id(
    identity: Identity,
    params: web::Path<(i32,)>,
) -> Result<impl Responder, DatabaseError> {
    let mut trx = db!().begin().await?;
    ...
    let cost = sqlx::query!(
        "SELECT creditor_id, receit_id FROM costs WHERE id=$1;",
        params.0
    )
    .fetch_one(&mut trx)
    .await?;
    ...
    let formerupload = if let Some(formerupload_id) = cost.receit_id {
        Some(
            sqlx::query!(
                "SELECT id, extension, original_filename, size_kb FROM uploads WHERE id=$1",
                formerupload_id
            )
            .fetch_one(&mut trx)
            .await?,
    ...
    let _query_res = sqlx::query!("DELETE FROM costs WHERE id = $1", params.0)
        .execute(&mut trx)
        .await?;
...
async fn post_wg_costs_balance(
    ...
    let result = sqlx::query!(
        "UPDATE costs SET equal_balances=$1 WHERE equal_balances IS NULL AND wg_id=$2;",
        id,
        wg_id
    )
    .execute(&mut trx)
...
async fn get_wg_costs_balance(identity: Identity) -> Result<impl Responder, DatabaseError> {
    let costs_opt = if let Some(wg_id) = identity.wg {
    ...
    Ok(HttpResponse::Ok().json(costs_opt))
}

#[get("/my_wg/costs/over_time/{interval}")]
async fn get_wg_costs_over_time(
    member: WGMemberIdentity,
    ...
    let balances_clean: Vec<RegularSpending> = balances
        .iter()
        .filter_map(|item| {
    ...
    Ok(HttpResponse::Ok().json(balances_clean))
}
